...
export default function UserCharts() {
    const [active, setActive] = useState("affiliation");

    return(
        <div className="metrics">
            <div id="categories">
                <button className="chart-button" onClick={() => setActive("affiliation")}>Total Users per Affiliation</button>

            </div>
            <div id="charts">
                {active === "affiliation" &&  <TotalBarChart chart_data={datasets_per_topic} chart_labels={topics}/>}
            </div>
        </div>
    )
};
